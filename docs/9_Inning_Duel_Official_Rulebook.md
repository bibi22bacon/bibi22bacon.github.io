...
Before the game, each player drafts a roster of **12–25** cards under a shared **salary cap of $1,000**. Printed card salaries use a **floor of $12** and a **ceiling of $200**.

Printed salaries come from a **2-AI draft market heated by head-to-head game simulations**, then remapped so the $1000 cap bites. See `data/SALARY_FORMULA.md` and `tools/sim_economy.mjs`.
...
### 8.1 Matrix layout (pitcher ↔ batter gradient)

Outcome cells are arranged with a consistent spatial bias:

- **Upper-left** favors the **pitcher** (`SO`, `FO`, `GO`)
- **Lower-right** favors the **batter** (`BB`, `1B`, `2B`, `HR`)

Cell counts still match each card’s printed outcome rates; only placement follows the gradient.

This is why tactic cards modify **D2**:
...
This edition preserves the draft’s structure while correcting language and keeping rules aligned with the living web-game implementation (Pickoff **D2 +1**, Hit & Run SO → lead-runner steal / steal-of-home auto out, salary floor **$12** / ceil **$200**).

### Playability clarifications

1. **Matrix row mapping** for D1 = 11–20 → row = D1 − 10.
2. **Matrix fill gradient**: upper-left → pitcher results; lower-right → batter results (see §8.1).
3. **Transformation order**: defensive tactic effects, then offensive tactic effects.
4. **Extra innings** and walk-off / bottom-of-9th standard baseball practice stated explicitly.
5. Eligible positions on cards may include **DH** in addition to C / 1B / 2B / 3B / SS / OF.

### Steal & bunt calibration (v1.2)

Dice and card ratings were simulated against 2025 card SPD/DEF distributions.

**Steal:** `Check = D1 + Catcher DEF − D2 − Runner SPD`

| Attempt | Safe if | Typical (SPD 6 vs Catcher DEF 4) |
|---------|---------|----------------------------------|
| **2B** | Check ≤ **9** | ~82% |
| **3B** | Check ≤ **6** | ~68% |
| **Home** (Hit & Run SO only, lead runner on 3B) | — | **Automatic out** |

| Matchup (2B cutoff) | Approx. success |
|---------|-----------------|
| SPD 8 vs Catcher DEF 4 (fast) | ~90% |
| SPD 2 vs Catcher DEF 6 (slow) | ~52% |
| League-wide vs typical catchers (2B) | ~75–78% (near MLB 2025) |
